...
async function Data() {
  const paslon = await prisma.ruangan.findMany({
    orderBy: { nama: "asc" },
  });
  return paslon;
}
...
async function Post(data: any) {
  if (String(data.get("mode")) == "add") {
    ...
    return { error: false, message: "Data ruangan berhasil ditambahkan" };
    ...
    return { error: false, message: "Data ruangan berhasil diperbarui" };
  } else {
    await prisma.ruangan.delete({
      where: { id: Number(data.get("id")) },
    });
    return { error: false, message: "Data ruangan berhasil dihapus" };
  }
}
